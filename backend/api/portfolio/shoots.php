...
if ($method === 'GET') {
    ...
    if (isset($_GET['id'])) {
        ...
        $shoot = $stmt->fetch();
        if (!$shoot) json_error('Shoot not found.', 404);
        $shoot['images'] = fetch_shoot_images($pdo, (int) $shoot['id'], true);
        json_success($shoot);
    }
    ...
    if (isset($_GET['category']) && isset($_GET['shoot'])) {
        $stmt = $pdo->prepare(
            'SELECT s.*, c.name AS category_name, c.slug AS category_slug
             FROM portfolio_shoots s
             JOIN portfolio_categories c ON c.id = s.category_id
             WHERE c.slug = :cat AND s.slug = :shoot AND s.visible = 1 AND c.visible = 1
             LIMIT 1'
        );
        $stmt->execute(['cat' => $_GET['category'], 'shoot' => $_GET['shoot']]);
        $shoot = $stmt->fetch();
        if (!$shoot) json_error('Shoot not found.', 404);
        $shoot['images'] = fetch_shoot_images($pdo, (int) $shoot['id']);
        json_success($shoot);
    }
    ...
    if (isset($_GET['category'])) {
        $stmt = $pdo->prepare(
            'SELECT s.*,
                    (SELECT image_path FROM portfolio_images pi WHERE pi.shoot_id = s.id AND pi.is_cover = 1 LIMIT 1) AS cover_image_path
             FROM portfolio_shoots s
             JOIN portfolio_categories c ON c.id = s.category_id
             WHERE c.slug = :cat AND s.visible = 1 AND c.visible = 1
...
if ($method === 'POST') {
    ...
    $v->required('category_id')->required('title', 'a shoot title')->maxLength('title', 160);
    ...
    $slug = $slugBase;
    $i = 1;
    $check = $pdo->prepare('SELECT COUNT(*) FROM portfolio_shoots WHERE category_id = :c AND slug = :s');
    do {
        $check->execute(['c' => (int) $input['category_id'], 's' => $slug]);
        if ((int) $check->fetchColumn() === 0) break;
        $slug = $slugBase . '-' . (++$i);
    } while (true);

    $stmt = $pdo->prepare(
        'INSERT INTO portfolio_shoots (category_id, title, slug, description, location, shoot_date, visible)
         VALUES (:cat, :title, :slug, :desc, :loc, :date, 1)'
    );
    $stmt->execute([
    ...
    $row = $pdo->prepare('SELECT * FROM portfolio_shoots WHERE id = :id');
    $row->execute(['id' => $id]);
...
if ($method === 'PUT') {
    ...
    $v = new Validator($input);
    $v->required('id')->required('title', 'a shoot title');
    if ($v->fails()) json_error('Please fix the errors below.', 422, $v->errors());

    $stmt = $pdo->prepare(
        'UPDATE portfolio_shoots
         SET title = :title, description = :desc, location = :loc, shoot_date = :date,
             visible = :visible, category_id = :cat
         WHERE id = :id'
    );
    $stmt->execute([
        'title'   => clean_string($input['title']),
...
if ($method === 'DELETE') {
    ...
    require_csrf();
    $id = (int) ($_GET['id'] ?? 0);
    if (!$id) json_error('Missing shoot id.', 422);
    $stmt = $pdo->prepare('DELETE FROM portfolio_shoots WHERE id = :id');
    $stmt->execute(['id' => $id]);
